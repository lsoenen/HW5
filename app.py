...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/lsoenenHW5"
## Provided:
...
class UpdatePriorityForm(FlaskForm):
    new_priority = StringField('What is the new priority of the item?', validators=[Required()])
    submit = SubmitField('Update')
...
def update(item):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash("Updated priority of " + i.description)
        return redirect(url_for('all_lists'))
